...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        $pdo->prepare("UPDATE veiculos SET marca=?,modelo=?,cor=?,placa=?,proprietario=?,celular=?,foto=? WHERE id=?")
            ->execute([$marca,$modelo,$cor,$placa,$prop,$cel,$foto,$id]);
        flash('Veículo atualizado.');
    } else {
        $pdo->prepare("INSERT INTO veiculos (marca,modelo,cor,placa,proprietario,celular,foto) VALUES (?,?,?,?,?,?,?)")
            ->execute([$marca,$modelo,$cor,$placa,$prop,$cel,$foto]);
...
<div class="card" style="border-color:var(--laranja-4)">
  ...
  <table>
    <thead><tr><th>Foto</th><th>Veículo</th><th>Cor</th><th>Placa</th><th>Proprietário</th><th>Celular</th><th></th></tr></thead>
    <tbody>
...
<div class="card">
  ...
  <table>
    ...
    <?php endforeach; ?>
    <?php if(!$lista):?><tr><td colspan="7" class="muted">Nenhum veículo cadastrado.</td></tr><?php endif;?>
    </tbody>
  </table>
</div>
